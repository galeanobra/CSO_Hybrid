...
public class NPointCrossover<T> implements CrossoverOperator<Solution<T>> {
    ...
    private List<Solution<T>> doCrossover(List<Solution<T>> s) {
        ...
        if (!s.get(0).getClass().equals(DefaultBinarySolution.class)) {
            ...
            Check.that(((BitSet) mom.variables().get(0)).size() >= crossovers, "The number of crossovers is higher than the number of bits");
            int[] crossoverPoints = new int[crossovers];
            for (int i = 0; i < crossoverPoints.length; i++) {
                crossoverPoints[i] = randomNumberGenerator.nextInt(0, ((BitSet) mom.variables().get(0)).size() - 1);
            }

            for (int i = 0; i < ((BitSet) mom.variables().get(0)).size(); i++) {
                if (swap) {
                    ((BitSet) boy.variables().get(0)).set(i, ((BitSet) mom.variables().get(0)).get(i));
                    ((BitSet) girl.variables().get(0)).set(i, ((BitSet) dad.variables().get(0)).get(i));
                }
